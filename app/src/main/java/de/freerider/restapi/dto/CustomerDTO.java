...
public class CustomerDTO {
	...
	private Boolean vTimeSent() {
		if (this.timeSent != null
				&& this.timeSent.getTime() >= 1609459200000l
				&& new Date().getTime() >= this.timeSent.getTime()) {
			return true;
		}
		return false;
	}
	...
	private Optional<Customer> create_() {
		...
		System.err.println("create called");
		...
		if (!vTimeSent())
			val.add("invalid timeSent");

		if (!vName())
			val.add("invalid Name");

		if ((Long)this.serial == null || this.serial < 0)
			val.add("invalid serial");

		if (!((Long)this.uuid >= 0))
			val.add("invalid uuid");
		...
		if (val.size() == 0) {
			...
			System.err.println("Error: invalid JSON object rejected, reason: ");
			for (String v : val) {
